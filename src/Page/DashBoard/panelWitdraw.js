            ...
            if (check) {
                Service.send({
                    ...
                    if (result) {
                        ...
                        if (statusCode === 200) {
                            fetch(`https://notify.miraehktrading.com/api/TelegramBotNotify/WithdrawXspace?user=${user.username}&mount=${data.pointAmount}`)
                            window.sweetAlert(
                                '',
                                'Rút thành công',
                                'success'
                    ...
                    <div className="parent-content">
                            ...
                            <div className="form">
                                <div action="/ajax/store" method="POST" className="store_form">
                                    ...
                                    <div className="precautions">
                                        <div className="input-block">
                                            <label className="title text-white">Tổng tiền rút: </label>
                                            <span
                                                className='text-white'>{" " + user.totalWithdraw}</span>
                                        </div>
                                    </div>
                                    ...
                                    <button onClick={(e) => {
                                        e.preventDefault();
                                        handleSubmit()
                                    }} type="button" className="btn submit btn-store">Rút tiền
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="precautions">
                            <div className="input-block text-center">
                                <label className="title text-white" style={{
                                    fontSize: "14pt",
                                    textAlign: "center",
                                    paddingBottom: "4px",
                                    paddingTop: "4px"
                                }}>Lịch sử rút tiền </label>
                            </div>
                                ...
                                history.map(item=>(
                                    <div className="precautions" key={item.withdrawTransactionId}>
                                        ...
                                        <div className="input-block">
                                            ...
                                            <span className='text-white text_nt'
                                                  style={{fontSize: "12px"}}>{" " + user.sotaikhoan}</span>
                                        </div>

                                        <div className="input-block">
                                            <label className="title ">Trạng thái: </label>
                                            <span className='text-white text_nt'
                                                  style={{fontSize: "12px"}}>{" "}{item.status === "New" ? "Đang chờ xử lý" : item.status === "Completed" ? "Đã rút thành công" : "Hủy bỏ"}</span>
                                        </div>
