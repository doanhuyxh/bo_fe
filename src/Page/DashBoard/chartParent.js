...
function ChartParent({ className }) {
        ...
        if (topic === `USER_${userId}` && payload.message) {
          ...
          setTimeout(()=>{
            if (newData.result === "win") {
              toast.success(`Thắng ${number_to_price(newData.amount)} - Giao dịch ${moment().subtract(1, "m").format("YYYYMMDDHHmm")}`, {
                autoClose: 5000,
              })

            } else {
              toast.error(`Thua ${number_to_price(newData.amount)} - Giao dịch ${moment().subtract(1, "m").format("YYYYMMDDHHmm")}`, {
                autoClose: 5000,
              })
            }
  ...
  return (
    <div className={`chart member_center`}>
      {
        !loading ? (
      ...
      <section className="lightBox ">
        ...
        <div className="lightBox-body"> {/*彈跳光箱-確認交易*/} <div className="lightBox-panel" id="lightBoxConfirm">
          ...
          <div className="lightBox-content">
            ...
            <div className="lightBox-header">Quy tắc <div className="lightBox-close" style={{ position: 'absolute', textAlign: 'right', top: '3px', right: '18px' }}>
              <i class="fa fa-times" aria-hidden="true" onClick={() => { window.lightBoxClose() }}></i>
            </div>
            </div>
            <div className="lightBox-content introductions"> . <table className="lightBox-table">
              <tbody>
                <tr>
                  <td>
                    <h6 className='text-nowrap'>Giới thiệu</h6>
                  </td>
                  <td />
                </tr>
                <tr>
